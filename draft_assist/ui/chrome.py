...
def card(title: str | None = None,
    ...
    A corner is sized to ITSELF: the heading's spare width goes to a
    trailing stretch, so every corner here stays the size it asks for.
    Anything that needs the card's full width — anything that REFLOWS,
    which cannot work out how many columns it has room for if it is only
    ever handed the width it currently uses — belongs in the card's BODY
    under the heading, not in this slot.
